...
public class LabelImage {
    ...
    public static void main(String[] args) {
        ...
        final LabelImage classifier = getClassifier(modelDir);
        final String result = classifier.classify(imageFile);

        //
        System.out.println("Classified : " + result);
    }

    public LabelImage(final String modelDir) {
        //
    ...
    public String classify(final String imageFile) {
        //
        byte[] imageBytes = readAllBytesOrExit(Paths.get(imageFile));
        try (Tensor<Float> image = constructAndExecuteGraphToNormalizeImage(imageBytes)) {
            float[] labelProbabilities = executeInceptionGraph(graphDef, image);
            int bestLabelIdx = maxIndex(labelProbabilities);
            return String.format("%s <br />(%.2f%% likely)",
